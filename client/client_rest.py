...
SERVER_URL = 'http://localhost:8501/v1/models/custom_model:predict'
...
def main():
    ...
    # Write to json file
    # with open("./temptemp.json", "w") as write_file:
    #     json.dump(response.json(), write_file, indent=4)
    
    with open("./temp/temp_input.jpg", 'wb') as f:
        f.write(dl_request.content)
    
    # Show image
    # Input
    nparr = np.fromstring(dl_request.content, np.uint8)
    ...
    # # Save inferred image
    # with open("./temp/temp_output.png", "wb") as output_file:
    #     output_file.write(response_image)
    
    # asd = base64.b64decode(prediction)
    # nparr = np.fromstring(asd, np.uint8)
    # img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
